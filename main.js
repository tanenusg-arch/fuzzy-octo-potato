...
function initBot() {
    ...
    try {
        ...
        // Quick command menu: register Telegram commands and show the Commands menu button.
        async function configureCommandMenu() {
            try {
                const commands = [
                    { command: 'start', description: 'Open the home menu' },
                    { command: 'shop', description: 'Browse the shop' },
                    { command: 'wallet', description: 'View your wallet and balance' },
                ...
                console.log('Telegram quick command menu configured successfully.');
            } catch (err) {
                console.error('Failed to configure Telegram quick command menu:', err?.message || err);
            }
        }

        // Quick commands. These render the same sections as the inline menu.
        bot.onText(/^\/shop(?:@\w+)?$/i, async (msg) => {
            await (0, store_1.awaitHydration)();
            if (msg.chat.type !== 'private') return;
            const chatId = msg.chat.id;
            if (checkUserBanned(chatId)) return;
            const products = await getProducts();
            const page = 1;
            const pageSize = 8;
            const pageProducts = products.slice(0, pageSize);
            let text = '<b>Shop</b>\n\n';
            if (!pageProducts.length) text += 'No products are currently available.';
            else pageProducts.forEach((prod, i) => {
                text += `${i + 1}. <b>${escapeHtml(prod.name)}</b> — <b>${prod.price} ETB</b> — Stock: <b>${prod.stock !== undefined && prod.stock !== null ? prod.stock : 'Unlimited'}</b>\n`;
            });
            const keyboard = pageProducts.map(prod => [{ text: prod.name.slice(0, 45), callback_data: `prod_view_${prod.id}` }]);
            keyboard.push([{ text: 'Refresh Stock', callback_data: 'btn_products_refresh' }]);
            await safeSendMessage(chatId, text, { reply_markup: { inline_keyboard: keyboard } });
        });

        bot.onText(/^\/wallet(?:@\w+)?$/i, async (msg) => {
            await (0, store_1.awaitHydration)();
            if (msg.chat.type !== 'private') return;
            const chatId = msg.chat.id;
            if (checkUserBanned(chatId)) return;
            const user = (0, store_1.getUser)(`tg_${chatId}`);
            await safeSendMessage(chatId, `<b>Vendra Sub Wallet Center</b>\n\nCurrent Balance: <code>${user.balance.toFixed(2)} ETB</code>\nTotal Spent: <code>${user.totalSpent.toFixed(2)} ETB</code>`, {
                reply_markup: { inline_keyboard: [[{ text: 'Deposit', callback_data: 'btn_wallet_deposit' }, { text: 'History', callback_data: 'btn_wallet_history' }], [{ text: 'Redeem', callback_data: 'btn_redeem' }], [{ text: 'Menu', callback_data: 'btn_main_menu' }]] }
            });
        });
        ...
        bot.onText(/^\/orders(?:@\w+)?$/i, async (msg) => {
            ...
            if (orders.length) {
                orders.slice(0, 4).forEach((o, i) => {
                    text += `${i + 1}. <code>${escapeHtml(o.id)}</code> - <b>${escapeHtml(o.productName)}</b>\nPrice: ${o.totalPrice} ETB | Status: [${escapeHtml(String(o.status || '').toUpperCase())}]\n\n`;
                });
            } else text += 'You have not placed any orders yet.';
            await safeSendMessage(chatId, text, { reply_markup: { inline_keyboard: [[{ text: 'Check Order', callback_data: 'btn_check_order_id' }], [{ text: 'Products', callback_data: 'btn_products_1' }, { text: 'Menu', callback_data: 'btn_main_menu' }]] } });
        });
        ...
        bot.onText(/^\/profile(?:@\w+)?$/i, async (msg) => {
            ...
            if (checkUserBanned(chatId)) return;
            const user = (0, store_1.getUser)(`tg_${chatId}`);
            await safeSendMessage(chatId, `<b>Your Profile</b>\n\n• User ID: <code>${escapeHtml(user.userId)}</code>\n• Username: <b>${escapeHtml(user.username)}</b>\n• Wallet Balance: <code>${user.balance.toFixed(2)} ETB</code>\n• Total Spent: <code>${user.totalSpent.toFixed(2)} ETB</code>\n• Referral Code: <code>${escapeHtml(user.referralCode)}</code>`, { reply_markup: { inline_keyboard: [[{ text: 'Referrals', callback_data: 'btn_referral' }, { text: 'Redeem', callback_data: 'btn_redeem' }], [{ text: 'Menu', callback_data: 'btn_main_menu' }]] } });
        });

        bot.onText(/^\/support(?:@\w+)?$/i, async (msg) => {
            await (0, store_1.awaitHydration)();
            if (msg.chat.type !== 'private') return;
            const chatId = msg.chat.id;
            if (checkUserBanned(chatId)) return;
            await safeSendMessage(chatId, `<b>Customer Support & Help Center</b>\n\nWe assist with order delivery, automated payment verification, and questions.\n\nOfficial Telegram Support: <a href=\"https://t.me/VendraSubET\">@VendraSubET</a>`, { reply_markup: { inline_keyboard: [[{ text: 'FAQ', callback_data: 'btn_faq' }], [{ text: 'Telegram Support', url: 'https://t.me/VendraSubET' }], [{ text: 'Menu', callback_data: 'btn_main_menu' }]] } });
        });
