...
function initBot() {
    ...
    try {
        // Initialize Telegram bot in webhook mode. Render receives Telegram updates at /webhook.
...
try {
  ...
  const server = http.createServer(async (req, res) => {
    if (req.method === 'GET' && (req.url === '/' || req.url === '/health')) {
      ...
      res.end(JSON.stringify({ ok: true, ...botModule.getBotHealth() }));
      return;
    }
  ...
  server.listen(PORT, '0.0.0.0', async () => {
    console.log(`Render webhook server listening on port ${PORT}`);
    const publicUrl = (process.env.RENDER_EXTERNAL_URL || '').replace(/\/$/, '');
    if (!publicUrl) {
      console.warn('RENDER_EXTERNAL_URL is not available yet; webhook was not registered automatically.');
      return;
    }
