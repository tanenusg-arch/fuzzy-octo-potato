...
async function placeQamifyOrder(productId, qty = 1) {
    ...
    try {
        ...
        // Qamify delivery extraction:
        // IMPORTANT: order.code is only Qamify's order reference (e.g. RA-XXXX),
        // never treat it as the product fulfillment. The actual delivery for
        // Qamify's documented order shape is order.items[].content.
        const fulfillmentValues = [];
        const seen = new Set();
        const ignoredKeys = new Set([
            'code', 'order_code', 'orderCode', 'id', 'order_id', 'orderId',
            'product_id', 'productId', 'product_name', 'productName', 'qty',
            'quantity', 'unit_price', 'unitPrice', 'unit_price_cents',
            'total', 'total_cents', 'currency', 'status', 'created_at',
            'createdAt', 'replayed', 'message', 'ok', 'balance', 'balance_cents'
        ]);
        const preferredKeys = new Set([
            'content', 'delivery', 'fulfillment', 'license_key', 'licenseKey',
            'license', 'key', 'keys', 'code_value', 'voucher', 'voucher_code',
            'voucherCode', 'redeem_code', 'redeemCode', 'activation_code',
            'activationCode', 'activation_key', 'activationKey', 'activation_link',
            'activationLink', 'redeem_link', 'redeemLink', 'subscription_link',
            'subscriptionLink', 'service_link', 'serviceLink', 'download_link',
            'downloadLink', 'purchase_link', 'purchaseLink', 'url', 'link',
            'credentials', 'credential', 'account', 'account_details',
            'accountDetails', 'login', 'username', 'email', 'password', 'token',
            'access_token', 'accessToken', 'value', 'output', 'response'
        ]);
        const looksLikeDeliveryString = (value) => {
            if (typeof value !== 'string') return false;
            const v = value.trim();
            if (!v || v.length > 20000) return false;
            if (/^https?:\/\//i.test(v)) return true;
            if (v.includes('|')) return true;
            if (/^[A-Za-z0-9][A-Za-z0-9._-]{3,}(?:-[A-Za-z0-9._-]{2,}){1,}$/.test(v)) return true;
            return false;
        };
        const addValue = (value, path, priority = 0) => {
            if (value === undefined || value === null || value === '') return;
            if (typeof value === 'string') {
                const v = value.trim();
                if (!v || seen.has(v)) return;
                seen.add(v);
                fulfillmentValues.push({ value: v, path, priority });
                return;
            }
            if (Array.isArray(value)) {
                for (const item of value) addValue(item, path, priority);
                return;
            }
            if (typeof value === 'object') {
                // Credential objects are delivered as readable key/value pairs.
                const pairs = Object.entries(value)
                    .filter(([k, v]) => v !== undefined && v !== null && typeof v !== 'object' && !ignoredKeys.has(k))
                    .map(([k, v]) => `${k}: ${String(v)}`);
                if (pairs.length) {
                    const joined = pairs.join('\n');
                    if (!seen.has(joined)) {
                        seen.add(joined);
                        fulfillmentValues.push({ value: joined, path, priority });
                    }
                }
                for (const [k, v] of Object.entries(value)) {
                    walk(v, `${path}.${k}`, preferredKeys.has(k) ? priority + 2 : priority);
                }
            }
        };
        const walk = (value, path = 'response', priority = 0) => {
            if (value === undefined || value === null) return;
            if (typeof value === 'string') {
                if (looksLikeDeliveryString(value)) addValue(value, path, priority);
                return;
            }
            if (Array.isArray(value)) {
                value.forEach((item, i) => walk(item, `${path}[${i}]`, priority));
                return;
            }
            if (typeof value === 'object') {
                for (const [key, child] of Object.entries(value)) {
                    if (ignoredKeys.has(key)) continue;
                    const childPriority = preferredKeys.has(key) ? priority + 10 : priority;
                    if (typeof child === 'string' && (preferredKeys.has(key) || looksLikeDeliveryString(child))) {
                        addValue(child, `${path}.${key}`, childPriority);
                    } else {
                        walk(child, `${path}.${key}`, childPriority);
                    }
                }
            }
        };

        // Qamify's actual delivery field. This is deliberately first and wins.
        const items = Array.isArray(resData?.order?.items) ? resData.order.items : [];
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (item && item.content !== undefined && item.content !== null && String(item.content).trim()) {
                addValue(item.content, `order.items[${i}].content`, 1000);
            }
        }
        // Fallback for other product delivery shapes.
        walk(resData, 'response', 0);

        fulfillmentValues.sort((a, b) => b.priority - a.priority);
        const licenseKey = fulfillmentValues[0]?.value;
        if (!licenseKey) {
            console.warn('Qamify order completed but no fulfillment content was found. Response keys:', Object.keys(resData || {}));
        }
        return {
            success: true,
            ...
            data: resData,
            licenseKey,
            orderId: resData.order?.code || resData.order?.id || resData.id || resData.order_id || resData.data?.id
        };
    }
    catch (err) {
