...
function initBot() {
    ...
    try {
        // Initialize Telegram bot in long-polling mode only. No HTTP server or port is used.
...
const PUBLIC_URL = String(process.env.RENDER_EXTERNAL_URL || process.env.WEBHOOK_URL || '').replace(/\/$/, '');
...
try {
  ...
  const server = http.createServer((req, res) => {
    if (req.method === 'GET' && req.url === '/') {
      ...
      res.end(JSON.stringify({ ok: true, service: 'VendraSub', mode: 'webhook' }));
      return;
    }
  ...
  server.listen(PORT, '0.0.0.0', async () => {
    ...
    if (!PUBLIC_URL) {
      console.error('RENDER_EXTERNAL_URL or WEBHOOK_URL is required for webhook mode.');
      return;
    }
